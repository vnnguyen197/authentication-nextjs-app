...
export default function Register(props: IRegisterProps) {
  ...
  const onSubmit = async (dt: any) => {
    console.log(dt);
    const { data: token } = await authAPI.getRegister(dt);
    try {
    ...
    <h1 className={style.register__title}>Register</h1>
      ...
      <div className={style.register__content}>
        <div className={style.register__user}>
          <label className={style["register__user--name"]} style={{marginRight:'10px'}}>First Name</label>
          <input type="text" {...register("firstname", { required: "Please enter your first name.",})}/>
        </div>
        <div className={style.register__pass}>
          <label className={style["register__user--name"]} style={{marginRight:'15px'}}>Last Name</label>
          <input type="text" {...register("lastname", { required: "Please enter your last name.",})}/>
        </div>
        <div className={style.register__user}>
          <label className={style["register__user--name"]} style={{marginRight:'10px'}}>Username</label>
          <input type="text" {...register("username", { required: "Please enter your user name.",})}/>
        </div>
        <div className={style.register__pass}>
          <label className={style["register__user--name"]} style={{marginRight:'15px'}}>Password</label>
          <input type="password" {...register("password", { required: "Please enter your pass word.",})}/>
        </div>
      </div>
      <div className={style.register__button}>
        <button className={style["register__button--btn"]} type="submit" onSubmit={handleSubmit(onSubmit)}>
          REGISTER
        </button>
      </div>
      </form>
